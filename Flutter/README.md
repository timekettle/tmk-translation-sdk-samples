...
目标依赖为 `tmk_translation_flutter: 1.3.1-rc.4`。Sample 的 `pubspec.lock`（含校验和）已锁定 pub.dev 发布的 RC4，iOS `Podfile.lock` 已锁定对应的原生 RC4。开发联调可在本机创建未跟踪的 `pubspec_overrides.yaml` 使用相对 path 依赖，但不得提交该文件或绝对路径；正式验收应使用发布制品。
