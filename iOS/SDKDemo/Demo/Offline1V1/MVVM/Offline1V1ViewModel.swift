...
struct OfflineSourceLanguagePickerValidation: Equatable {
    let canConfirm: Bool
    let message: String?
}

enum OfflineSourceLanguagePickerValidationFactory {
    static func make(candidateSourceLanguage: String,
                     targetLanguage: String,
                     selectedScenarioOption: OfflineScenarioOption,
                     isScenarioUpdating: Bool,
                     pendingScenarioOption: OfflineScenarioOption?) -> OfflineSourceLanguagePickerValidation {
        let roomScenario: TmkRoomScenario
        if isScenarioUpdating, let pendingScenarioOption {
            roomScenario = pendingScenarioOption.roomScenario
        } else {
            roomScenario = selectedScenarioOption.roomScenario
        }
        let canConfirm = OneToOneSameLanguagePolicy.isOfflineAllowed(
            source: candidateSourceLanguage,
            target: targetLanguage,
            roomScenario: roomScenario
        )
        return OfflineSourceLanguagePickerValidation(
            canConfirm: canConfirm,
            message: canConfirm ? nil : OneToOneSameLanguagePolicy.requiresRecognizeMessage
        )
    }
}
...
final class Offline1V1ViewModel: NSObject {
    let authVerifyMode: TmkAuthVerifyMode = .offline
    ...
    func sourceLanguagePickerValidation(for candidateSourceLanguage: String) -> OfflineSourceLanguagePickerValidation {
        OfflineSourceLanguagePickerValidationFactory.make(
            candidateSourceLanguage: candidateSourceLanguage,
            targetLanguage: selectedTargetLang,
            selectedScenarioOption: selectedScenarioOption,
            isScenarioUpdating: isScenarioUpdating,
            pendingScenarioOption: pendingScenarioOption
        )
    }
...
private extension Offline1V1ViewModel {
    ...
    func createOfflineChannel() {
        ...
        let roomConfig = TmkTranslationRoomConfig(mode: .offline,
                                                   ...
                                                   dialogConversationAudioMode: channelAudioMode)
        updateStatus("正在创建离线房间...")
        TmkTranslationSDK.shared.createTmkTranslationRoom(config: roomConfig) { [weak self] roomResult in
            ...
            case .success(let room):
                let builder = TmkTranslationChannelConfig.Builder()
                    ...
                    .setSourceLang(self.selectedRightLang)
                    .setTargetLang(self.selectedLeftLang)
                    .setSampleRate(16_000)
                    .setChannelNum(self.channelModeConfiguration.pcmChannels)
                    .setChannelAudioMode(self.channelAudioMode)
                    .setTTSAudioCallbackThread(.background)
                    .setModelRootDirectory(self.modelRootDirectory)
                    .setTranslateMode(self.selectedTranslateMode)
                    .setRoomScenario(self.selectedScenarioOption.roomScenario)
                if speakers.isEmpty == false {
                    _ = builder.setSpeakers(speakers)
                }
                let config = builder.build()
                self.updateStateOnMain {
                    $0.configuredSampleRate = config.sampleRate
                    $0.configuredChannels = config.channelNum
                }
                self.updateStatus("正在加载离线模型...")
            ...
            case .failure(let error):
                self.downloadButtonState = .notDownloaded
                self.checkModelReadyAndUpdateButton(autoStartIfReady: false, autoDownloadIfNeeded: false)
                self.updateStatus("离线房间创建失败：\(error.localizedDescription)")
            }
        }
    }
