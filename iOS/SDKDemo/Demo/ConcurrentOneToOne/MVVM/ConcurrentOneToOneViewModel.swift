...
final class ConcurrentOneToOneViewModel: NSObject {
    ...
    let authVerifyMode: TmkAuthVerifyMode = .all
    ...
    func start() {
        ...
        // 不在 AudioUnit realtime callback 中执行交错或 SDK 推流，避免音频线程被旁路处理拖慢。
    ...
    private func prepareOnlineRuntime(generation: UInt64) {
        ...
        let roomConfig = TmkTranslationRoomConfig(mode: .online, sourceLang: rightLanguage, targetLang: leftLanguage, roomScenario: profile.roomScenario, channelScenario: .oneToOne, speakers: speakers, onlineTranslateEngine: profile.translateEngine, onlineRecognizeEngine: profile.recognizeEngine, translateMode: profile.translateMode, dialogConversationAudioMode: profile.audioMode, enableSensitiveWordRedaction: redaction)
        onlineRoomOperation = TmkTranslationSDK.shared.createTmkTranslationRoom(config: roomConfig) { [weak self] result in
            ...
            case .success(let room):
                guard self.isCurrent(.online, generation: generation) else { return }
                let config = TmkTranslationChannelConfig.Builder().setRoom(room).setScenario(.oneToOne).setMode(.online).setSourceLang(self.rightLanguage).setTargetLang(self.leftLanguage).setSpeakers(speakers).setSampleRate(OneToOneDemoDefaults.sampleRate).setChannelNum(OneToOneDemoDefaults.channelCount).build()
    ...
    private func prepareOfflineRuntime(generation: UInt64) {
        ...
        let speakers = [TmkSpeaker(channel: .left, gender: profile.leftSpeaker), TmkSpeaker(channel: .right, gender: profile.rightSpeaker)]
        let roomConfig = TmkTranslationRoomConfig(mode: .offline,
                                                   sourceLang: rightLanguage,
                                                   targetLang: leftLanguage,
                                                   roomScenario: .toSpeech,
                                                   channelScenario: .oneToOne,
                                                   speakers: speakers,
                                                   translateMode: profile.translateMode,
                                                   dialogConversationAudioMode: profile.audioMode)
        TmkTranslationSDK.shared.createTmkTranslationRoom(config: roomConfig) { [weak self] roomResult in
            guard let self, self.isCurrent(.offline, generation: generation) else { return }
            switch roomResult {
            case .success(let room):
                let config = TmkTranslationChannelConfig.Builder().setRoom(room).setScenario(.oneToOne).setMode(.offline).setSourceLang(self.rightLanguage).setTargetLang(self.leftLanguage).setSpeakers(speakers).setSampleRate(OneToOneDemoDefaults.sampleRate).setChannelNum(OneToOneDemoDefaults.channelCount).setChannelAudioMode(profile.audioMode).setTTSAudioCallbackThread(.background).setModelRootDirectory(OneToOneDemoDefaults.offlineModelRootDirectory()).setTranslateMode(profile.translateMode).setRoomScenario(.toSpeech).build()
                let listener = Listener(owner: self, runtime: .offline, generation: generation)
                self.offlineListener = listener
                self.offlineChannelOperation = TmkTranslationSDK.shared.createTranslationChannel(config, listener: listener) { [weak self] result in self?.bind(result, runtime: .offline, generation: generation) }
            case .failure(let error):
                self.updateState(runtime: .offline, generation: generation) {
                    $0.offlineStatus = "建房失败：\(error.message)"
                    $0.offlineCanRetry = true
                }
            }
        }
    ...
    private func enqueueAudioInput(_ data: Data, sessionGeneration: UInt64) {
        ...
        audioInputQueue.async { [weak self] in
            ...
            self.routeAudio(mic: data, sessionGeneration: sessionGeneration)
        }
    }
